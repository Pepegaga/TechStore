...
router.post('/cart', async(req,res)=>{
    ...
    try {
        ...
        const list = await pool.query("SELECT * FROM cart WHERE user_id = $1",[id])
...
router.post('/addToCart', async(req,res)=>{
    ...
    try {
        ...
        const flag = await pool.query("SELECT * FROM cart WHERE product_id=$1 AND user_id=$2",[prodId,userId])
        console.log(flag)
        if(flag.rowCount>0){
            res.json({flag: false})
        }else{
...
router.delete("/deleteFromDB", async (req, res) => {
    try {
      ...
      await pool.query('DELETE FROM cart WHERE user_id = $1 AND product_id=$2', [ id, prodId ]);
