...
router.put('/editPass', async (req, res) => {
    try{            
        ...
        await pool.query(
            "UPDATE users SET user_pass = $1 WHERE user_id = $2",
            [hashPassword, id]
        );
...
router.put('/editName', async (req, res)=>{
    try {
        ...
        res.status(201).json({message: 'Name edited',name: user.rows[0].user_name })
...
router.put('/editAddress', async (req, res)=>{
    try {
        ...
        res.status(201).json({message: 'Address edited', address: user.rows[0].user_address})
