...
router.post(
    ...
    [
        ...
        check('password', 'minimum lenght of password is 6 letters')
        .isLength({min: 6})
    ],
     async (req,res)=>{
    try {
        ...
        const candidate = await pool.query("SELECT * FROM users WHERE user_login = $1", [ login ]);
...
router.post(
    ...
    async (req,res)=>{
        try {
            ...
            const user = await pool.query("SELECT * FROM users WHERE user_login = $1", [ login ]);
            ...
            res.json({token, id: user.rows[0].user_id, name: user.rows[0].user_name, address: user.rows[0].user_address})
...
router.put('/forgot', async(req,res)=>{
    try {
        ...
        await pool.query("UPDATE users SET user_pass=$1 WHERE user_id=$2",[hashPassword,id])
...
router.post('/', async (req, res) => {
    try{
        ...
        const allData = await pool.query("SELECT * FROM users WHERE user_login=$1", [login]);
        res.json({login: allData.rows[0].user_login, id:allData.rows[0].user_id });
    }catch(e){
