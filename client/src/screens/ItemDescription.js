...
export default function ItemDescription({ route, navigation }) {
    ...
    const addToCartHandler = async () => {
        try {
            ...
            if (flag.flag) {
                ...
                const rerender2 = useSelector(
                    (state) => state.cart.rerenderTemp
                )
                alert(rerender2)
            }
        } catch (error) {}
    ...
    const addToFavouriteHandler = async () => {
        try {
            const flag = await request(
                `${baseUrl}/api/favourite/addToFavourite`,
                'POST',
                { userId, prodId }
            )
            if (flag.flag > 0) {
    ...
    const isFavouriteHandler = async () => {
        try {
            const fav = await request(
                `${baseUrl}/api/favourite/isFavourite`,
                'POST',
                { userId, prodId }
            )
            if (fav.flag > 0) {
    ...
    return (
        <View>
            ...
            <Header
                ...
                centerComponent={
                    <Text style={styles.headerText}>{item.product_name}</Text>
                }
            />
