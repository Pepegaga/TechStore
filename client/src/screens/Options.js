...
export default (props) => {
    ...
    const passwordEditHadler = async () => {
        try {
            await request(`${baseUrl}/api/options/editPass`, 'PUT', {
                id,
                currentPass,
                newPass,
            })
    ...
    const nameEditHadler = async () => {
        try {
            const newName = await request(
                `${baseUrl}/api/options/editName`,
                'PUT',
                { id, name }
            )
    ...
    const addressEditHadler = async () => {
        try {
            const newAddress = await request(
                `${baseUrl}/api/options/editAddress`,
                'PUT',
                { id, address }
            )
            AsyncStorage.setItem(
        ...
        <View style={styles.container}>
            ...
            >
                <ScrollView>
                    ...
                    <Text
                        style={{ marginLeft: 20, marginTop: 20, fontSize: 16 }}
                    >
                        Name:{' '}
                    </Text>
                    <TextInput
